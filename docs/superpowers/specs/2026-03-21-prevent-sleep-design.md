...
Add a "prevent sleep" toggle to Settings > General tab, preventing the system from sleeping while B4Claw is running. Follows the existing autoLaunch pattern.

## User Story

As a user running long AI tasks or IM bots, I want B4Claw to prevent my computer from sleeping, so tasks aren't interrupted by the system going to sleep.
