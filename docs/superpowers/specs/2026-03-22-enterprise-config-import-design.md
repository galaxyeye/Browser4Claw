...
B4Claw needs enterprise batch deployment support: IT/ops teams push a unified config file to multiple machines, and B4Claw loads it on startup. The config file reuses OpenClaw's `openclaw.json` format so that ops teams maintain a single config for both OpenClaw and B4Claw deployments.
...
OpenClaw uses plugin-style channel keys that differ from B4Claw's internal platform names. The importer accepts **both** forms for flexibility:

| openclaw.json channel key | B4Claw im_config key |
|---------------------------|------------------------|
