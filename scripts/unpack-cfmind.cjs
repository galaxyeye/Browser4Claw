 ...
 * 通过 B4Claw.exe (ELECTRON_RUN_AS_NODE=1 模式) 执行。
 *
 * 用法: B4Claw.exe <本脚本路径> <tarPath> <destDir>
 *
 * 效果:
...
if (!tarPath || !destDir) {
  console.error('[unpack-cfmind] Usage: B4Claw.exe unpack-cfmind.cjs <tarPath> <destDir>');
  process.exit(1);
}
