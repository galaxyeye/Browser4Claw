...
**Issue**: Need to verify this warmup is actually succeeding on Windows. Check `%APPDATA%\B4Claw\install-timing.log` for `warmup-done` entry and verify the cache directory is populated.
