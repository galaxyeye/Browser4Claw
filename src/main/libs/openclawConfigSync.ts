...
const MANAGED_WEB_SEARCH_POLICY_PROMPT = [
  ...
  '- Only use the B4Claw `web-search` skill when local command execution is available. Native channel sessions may deny `exec`, so prefer `browser` or `web_fetch` there.',
  ...
  '',
  'Do not claim you searched the web unless you actually used `browser`, `web_fetch`, or the B4Claw `web-search` skill.',
].join('\n');
...
export class OpenClawConfigSync {
  ...
  /**
   * Resolve the B4Claw SKILLs installation directory for OpenClaw's
   * `skills.load.extraDirs` configuration.
   *
   * Cross-platform paths (via Electron app.getPath('userData')):
   *   macOS:   ~/Library/Application Support/B4Claw/SKILLs
   *   Windows: %APPDATA%/B4Claw/SKILLs
   *   Linux:   ~/.config/B4Claw/SKILLs
   */
  ...
  /**
   * Build per-skill `enabled` overrides from the B4Claw SkillManager state,
   * so that skills disabled in the B4Claw UI are also hidden from OpenClaw.
   */
