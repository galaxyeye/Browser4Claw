...
export class OpenClawRuntimeAdapter extends EventEmitter implements CoworkRuntime {
   ...
   * so B4Claw can recover even when the gateway fails to deliver the abort event.
   */
   ...
   * for a B4Claw-initiated session.
   */
  ...
  private buildSystemPromptPrefix(systemPrompt: string): string {
    return [
      '[B4Claw system instructions]',
      'Apply the instructions below as the highest-priority guidance for this session.',
      'If earlier B4Claw system instructions exist, replace them with this version.',
      systemPrompt,
    ].join('\n');
  }
  ...
  private buildBridgePrefix(messages: CoworkMessage[], currentPrompt: string): string {
    ...
    return [
      '[Context bridge from previous B4Claw conversation]',
   ...
   * gateway rather than the B4Claw UI.
   *
